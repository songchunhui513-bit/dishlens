...
export default function DishDetailPage({ dish, onBack, onReview, showAllergens, targetLang = "zh", uiLang = "zh", isFavorited, onToggleFavorite, onShare, imageGenProgress, imageGenerating = false, onGenerateImage, smartTags = [], orderQuantity = 0, orderTotalQuantity = 0, orderTotalLabel = "价格待核对", restaurantSource, onOrderQuantityChange, onOpenOrderConfirm }: DishDetailPageProps) {
  ...
  if (!dish) {
    return (
      <div className="h-full flex flex-col items-center justify-center" style={{ background: "var(--bg)" }}>
        <div style={{ fontFamily: "var(--font-body)", fontSize: 14, color: "var(--muted)" }}>暂无菜品数据</div>
        <button onClick={onBack} style={{ marginTop: 12, fontFamily: "var(--font-ui)", fontSize: 13, color: "var(--primary)", background: "none", border: "none", cursor: "pointer" }}>← 返回</button>
      </div>
    );
  }
    ...
    <div className="h-full flex flex-col relative" style={{ background: "var(--bg)" }}>
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 flex-shrink-0" style={{ background: "rgba(255,250,242,0.9)", borderBottom: "1px solid rgba(232,213,192,0.72)", backdropFilter: "blur(16px)" }}>
        ...
        <button
          ...
          className="flex items-center gap-0.5 text-[12px] font-bold transition-all duration-200"
        ...
        <div style={{ padding: "0 16px 16px" }}>
          ...
          <div style={{ marginBottom: 16 }}>
            <DishImageWithLoading
              dish={dish}
              size="hero"
              alt={dishText.translatedName}
              pendingDone={imageGenProgress?.done}
              pendingTotal={imageGenProgress?.total}
              pendingActiveTotal={imageGenProgress?.activeTotal}
              pendingQueuedTotal={imageGenProgress?.queuedTotal}
              onRetry={() => onGenerateImage?.(dish)}
              retrying={imageGenerating}
              priority
            >
            {isVeg && (
