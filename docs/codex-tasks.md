...
- [x] 用真实菜单建立线上冷启动与重复上传基线；单页 10 道首次约 21 秒、重复约 1 秒，四页 49 道首次完整文字约 36 秒、重复约 1.8 秒。
- [x] `partial` 结果禁止写入内存或文件缓存，避免缺页结果污染重复上传。
- [x] 四页 fast first-pass 默认并发从 4 调为 3，49 道线上复测 4/4 页完整返回。
...
- [x] 生产浏览器上传四页密集法文菜单，49 道完整返回，列表渐进渲染与详情进入/返回通过。
- [ ] 继续上传英文、意大利文、弱光、倾斜和快餐菜单。
