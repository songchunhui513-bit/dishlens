...
注意：iOS 系统分享面板里的微信入口由微信 App 的 Share Extension 接管，网页无法控制其打开、加载和失败状态。因此它只作为系统能力保留，不作为 DishLens 的微信主路径。
...
不使用私有 deep link，也不再调用 `navigator.share` 间接调起 iOS 微信分享扩展。微信按钮固定执行复制链接，并提示用户打开微信后粘贴给好友或群聊。

原因：普通 H5 页面没有稳定的公开能力直接打开微信分享页；iOS 原生分享面板里的微信扩展可能因微信登录态、扩展状态、系统缓存或微信自身异常卡住。复制链接是当前最可控、失败率最低的路径。
